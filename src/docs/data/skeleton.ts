...
export const skeletonDoc: ComponentDoc = {
	...
	props: [
		{
			...
			default: "'text'",
			note: 'Picks a shape and its default dimensions. Compose several Skeletons in your own Stack/Cluster/Card layout for a card-like placeholder — there is no dedicated preset prop.'
		},
		{
			name: 'width',
			type: 'string | number',
			default: 'variant default',
			note: 'A number is emitted as px; a string is used verbatim (any CSS length/percentage). Not a theme hook — a per-instance style, like Grid’s --hz-grid-cols*.'
		},
		{ name: 'height', type: 'string | number', default: 'variant default', note: 'See width.' },
		{
			...
			default: 'variant default',
			note: 'The shared Rounded scale. circle always forces full, regardless of this prop.'
		},
		{
			name: 'lines',
			type: 'number',
	...
	a11yNote:
		'Skeleton stamps `aria-hidden="true"` by default — it is decorative scaffolding with no role, no live region, and no label, so it cannot announce loading on its own. A consumer may override `aria-hidden` via rest for the rare case they want it exposed.\n\nThis is the deliberate Loading-announces / Skeleton-decorates division: wrap the loading region in `aria-busy="true"` and pair it with either a labelled indeterminate `Loading` or a polite `aria-live` "Loading…" message, so screen-reader users are told content is coming, then told when it arrives. See the paired loading pattern demo below.\n\n**Reduced motion is the opposite of Loading here, on purpose.** Skeleton\'s shimmer/pulse goes fully still under `prefers-reduced-motion: reduce`, because the placeholder shape itself already carries the "content loading here" cue — no motion is needed. `Loading` (and Button\'s loading spinner) instead keep animating, much more slowly, because for them motion *is* the essential cue.',
	a11yLinks: [
		{
			label: 'MDN: aria-busy',
