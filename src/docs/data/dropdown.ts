...
export const dropdownDoc: ComponentDoc = {
	...
	a11yNote:
		"The trigger is a real button (the library's own Button) with `aria-haspopup=menu`, `aria-expanded`, and `aria-controls`. It opens a `role=menu` popup of `role=menuitem` buttons, named by the trigger via `aria-labelledby`.\n\nUnlike `Combobox`, which keeps DOM focus on its text input and tracks a virtually-focused option with `aria-activedescendant`, Dropdown moves real DOM focus into the menu. Opening it lands focus on the first menuitem (the last, on `ArrowUp`), and a roving `tabindex` (`0` on the focused item, `-1` on the rest) keeps the menu a single tab stop — so the active item is styled with native `:focus` rather than a `data-active` hook.\n\nKeyboard: `ArrowDown`/`ArrowUp` move focus and wrap, including over disabled items. `Home`/`End` jump to the first/last item. Typing a character cycles focus to the next item whose label starts with it. `Enter`/`Space` activate the focused item through its own native button click, with no separate handling. `Escape` closes the menu and returns focus to the trigger. `Tab`/`Shift+Tab` close the menu and let focus move on — there is no focus trap.\n\nA disabled item keeps `aria-disabled=true` rather than the native `disabled` attribute, so it stays focusable and screen-reader users can discover it. Activating it is a no-op.",
	a11yLinks: [
		{
