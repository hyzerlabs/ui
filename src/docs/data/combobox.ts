...
export const comboboxDoc: ComponentDoc = {
	...
	a11yNote:
		"The visible input carries `role=combobox` with `aria-autocomplete=list`, `aria-expanded`, and `aria-controls` pointing at the `role=listbox` popup, which is `aria-multiselectable`. Instead of moving DOM focus into the list, the highlighted option is tracked as virtual focus with `aria-activedescendant` on the input — so screen readers announce the active option while the field stays editable.\n\n`description`/`error` chain into `aria-describedby` on the input (description first), still computed even while the popup visually overlays that region. `required` sets `aria-required`, and an `error` sets `aria-invalid`.\n\nEach selected value renders as a `Badge` chip inside the control. Every chip's labelled dismiss button (announced as Remove, followed by the option's label) is a real, individually-focusable tab stop, so tabbing into the field reaches the chips before the input.\n\nKeyboard: `ArrowDown`/`ArrowUp` open the popup and move the active option, wrapping and skipping disabled entries. `Alt+ArrowDown` opens without moving, and `Alt+ArrowUp` closes without changing anything. `Home`/`End` jump to the first/last enabled option while open (they're native text-cursor moves while closed). `Enter` toggles the active option's membership and keeps the popup open so you can keep picking. `Backspace` on an empty query removes the last chip. `Escape` closes the popup and clears the filter query only — it never clears a selection, since every chip has its own dismiss button. `Tab` closes the popup and lets focus move on.\n\nThe input is the only tab stop for opening the popup — the trailing toggle button is `tabindex=-1`, reachable by pointer only.",
	a11yLinks: [
