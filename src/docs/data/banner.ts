...
export const bannerDoc: ComponentDoc = {
	...
	a11yNote:
		'A statically rendered Banner is plain content — no role, no live region. For a Banner inserted after load, pass `role="status"` (polite) or `role="alert"` (assertive, use sparingly) via the rest props.\n\nThe icon slot is decorative (`aria-hidden`), links inside are underlined, and colour is never the only signal. The dismiss button is a real labelled `<button>` — dismissal is your state change.\n\nA pinned Banner can cover a focused element that scrolls under it. Keep pinned banners short (one line where possible), and give in-page anchor targets `scroll-margin-block-start`/`-end` equal to the banner height so they clear it. The Banner itself sets no `scroll-margin`.\n\nThere is deliberately no Toast component: timed self-dismissing overlays fail WCAG 2.2.1 and routinely escape announcement. Banner — with opt-in `role="status"` and consumer-owned dismissal — is the sanctioned pinned or site-wide pattern instead.',
	a11yLinks: [
		{
