...
export const dividerDoc: ComponentDoc = {
	...
	a11yNote:
		'The bare form is a native `<hr>` — implicit `role="separator"`, horizontal, non-focusable, and needing no ARIA.\n\nThe labelled form keeps `role="separator"` on the wrapping `<div>`, so the thematic break survives the switch away from `<hr>`. The label text is the separator\'s accessible name and stays in the tree, so screen readers announce \'separator, OR\'.\n\nThe flanking rules are decorative `::before`/`::after` pseudo-elements — never part of the accessibility tree.',
	a11yLinks: [
