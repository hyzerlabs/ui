...
export const sliderDoc: ComponentDoc = {
	...
	a11yNote:
		"The range input is the labelled (`id`/`for`), named, form-participating control. Its slider semantics (value, min, max) are native, and arrow keys step it.\n\nThe number field is an exact-entry affordance with its own accessible name (`inputLabel`) and no `name` — it never submits.\n\n`description` and `error` chain into `aria-describedby` on the range (description first), and an `error` sets `aria-invalid`. `required` renders the label indicator only — `aria-required` isn't part of the slider role's supported ARIA attributes, so it's never applied.\n\n`unit` is decorative and `aria-hidden` — put meaning-bearing units in the label or description instead. In vertical orientation the component stamps `aria-orientation=\"vertical\"` on the range explicitly, since writing-mode doesn't reliably flip a native range's implicit `horizontal` orientation in the accessibility tree across engines and assistive tech.",
	a11yLinks: [
