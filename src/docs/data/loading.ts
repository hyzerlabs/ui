	...
	props: [
		{
			...
			note: 'Omitted, undefined, or NaN ⇒ indeterminate. There is no separate `indeterminate` boolean: the mode follows `value`, the same way a native `<progress>` does.'
		},
		{ name: 'max', type: 'number', default: '100' },
		{
			...
			type: 'Intent',
			default: "'primary'",
			note: 'Defaults to primary, unlike Badge and Banner, since a fill reads as an accent. See Foundation → Colors & Intent.',
		...
		{ name: 'size', type: "'sm' | 'md' | 'lg'", default: "'md'" },
		{
			...
			default: "'bar'",
			note: 'A value on `bar` renders a determinate linear progress bar; a value on `ring` renders a determinate circular arc with a centered readout. `spinner` and `dots` are indeterminate only, so a value passed to either is ignored (dev-warn). With no value, `ring` rotates continuously while its arc grows and shrinks.'
		},
		{
			name: 'label',
			type: 'string',
			default: '—',
			note: 'The accessible name. Required: a bare progressbar has a role but no name, and a missing name dev-warns. You can pass aria-label or aria-labelledby through rest instead.'
		},
		{
			name: 'showValue',
			type: 'boolean',
			default: 'false',
			note: 'Renders a formatted readout beside the bar, or centered inside a determinate ring. Determinate only: ignored (dev-warn) on any indeterminate presentation (including the indeterminate ring) and on spinner/dots. At sm/md the centered ring readout gets tight, so prefer lg or a larger --hz-loading-size.'
		},
		{
			name: 'format',
			type: '(value: number, max: number) => string',
			default: '(v, max) => `${Math.round((v / max) * 100)}%`',
			note: 'Feeds both the visible readout and aria-valuetext, so what you see and what is announced never differ. Applies to both the bar and the determinate ring.'
		},
