<script lang="ts">
	...
	const tocLevelsByPath: Record<string, number[]> = {
		'/docs/foundation/colors': [2, 3],
		'/docs/foundation/utilities': [2, 3]
	};
...
<style>
	...
	.docs-main-inner {
		max-width: 56rem;
		/* Route transitions (onNavigate lives in the root layout): only the
		 * page content participates in the cross-fade; the root snapshot is
		 * pinned static below so the shell never moves. */
